...
if %input%==5 goto :Devstate
if %input%==6 ( exit
) else ( 
    echo: invalid reponse try again & goto :mainmenu
)
...
:Devstate
set currentlabel=:Devstate
echo:----------------------------------
echo:I will need to export this to a text file since there are 
echo:too many drivers to show in this cmd session...
echo:Continue? [y, n]
set /p continue=: 
if %continue%==n goto :mainmenu
if %continue%==y ( cd %userprofile%
    call :driveroutput >Desktop\Driveroutput.txt 
) else ( 
    echo:Invalid response try again & goto :Devstate 
)
